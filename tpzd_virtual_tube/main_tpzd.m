       ...
       for i = 1:N
           ...
           if leader_locate(i)~=0
              %% Velocity command component of moving along the virtual tube
               ...
               ci = k3*dmysigma2(rtksii(i)-norm(ksiti(i,:)),rs1,ra_V2(i,1),et);%ra_V2 change with density
          ...
          %% Velocity command component of collision avoidance
           ...
           if (Pcur(i,1) >= (curve_x_right-0.2) && Pcur(i,2)<= 1.5) || flag_goal(i) == 1
               V(i,:) = [0,vmax];
               flag_goal(i) = 1;
               for m = 1:N
                   if i ~= m && Pcur(m,2) > 0.1
